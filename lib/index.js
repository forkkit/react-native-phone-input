...
export default class PhoneInput extends Component {
  ...
  static getFlag = (iso) => {
    return Flags.get(iso)
  }
  
  static getCountryByISO = (iso2) => {
    return PhoneNumber.getCountryDataByCode(iso2);
  }
  
  static getCountryDialCodeByISO = (iso2) => {
    const country = PhoneNumber.getCountryDataByCode(iso2);
    return country ? country.dialCode : ""
  }
  
  static getCountryISOFromNumber = (number) => {
    return PhoneNumber.getCountryCodeOfNumber(number);
  }
  
  static formatNumber = (text, iso2) => {
      return PhoneNumber.format(text, iso2)
  }
  
  static isValidNumber(number, iso2) {
    return PhoneNumber.isValidNumber(number, iso2);
  }
  ...
  onPressFlag() {
    if (this.props.onPressFlag) {
      ...
      this.props.onFlagPressed && this.props.onFlagPressed()
  ...
  updateFlagAndFormatNumber(number, actionAfterSetState = null) {
    const { allowZeroAfterCountryCode, onFlagChange, initialCountry } = this.props;
    let formerIso = this.getISOCode() || initialCountry;
    let iso2 = formerIso;
    ...
    if (formerIso !== iso2){
      onFlagChange && onFlagChange(formerIso, iso2)
    }
  ...
  render() {
    ...
    const FlagCompoent = this.props.flagComponent || Image;
    return (
      <View style={[styles.container, this.props.style]}>
        ...
        <View style={[
          { flex: 1, marginLeft: this.props.offset || 10 },
